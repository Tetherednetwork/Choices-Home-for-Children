...
const SectionContent: React.FC<{
  ...
  const handleCheckboxChange = (questionId: string, optionValue: string, checked: boolean) => {
    ...
    const currentAnswers = Array.isArray(currentSelection) ? currentSelection.map(String) : [];
    
    let newAnswers;
    if (checked) {
        newAnswers = [...currentAnswers, optionValue];
    } else {
        newAnswers = currentAnswers.filter(item => item !== optionValue);
    }
              ...
              <div key={q.id}>
                ...
                <div className="mt-1 p-3 bg-slate-100 rounded-md text-sm text-slate-800 min-h-[40px] prose prose-sm max-w-none break-words">
                  {(() => {
                    ...
                    if (q.type === 'signature' && typeof answer === 'string' && answer.startsWith('data:image')) {
                        return <img src={answer} alt="Signature" className="max-w-xs border rounded-md" />;
                    }
                    if (q.type === 'rating') {
                        return <StarRating value={Number(answer)} onChange={() => {}} disabled={true} />;
                    }
                    if (q.type === 'file-upload' && answer && typeof answer === 'object' && 'name' in answer && 'size' in answer) {
                        return (
                            <div className="flex items-center gap-2 p-2 bg-slate-200 rounded-md max-w-sm">
                                ...
                                <span className="text-sm font-medium text-slate-800 truncate">{(answer as any).name}</span>
                                <span className="text-xs text-slate-500 ml-auto flex-shrink-0">({((answer as any).size / 1024).toFixed(1)} KB)</span>
                            </div>
                        );
                    }
                    if (q.type === 'url' && typeof answer === 'string') {
                        return <a href={!answer.startsWith('http') ? `https://${answer}`: answer} target="_blank" rel="noopener noreferrer" className="text-sky-700 hover:underline break-all">{answer}</a>;
                    }
                    if (Array.isArray(answer)) {
                        if (answer.length === 0) return <span className="text-slate-400">Not answered</span>;
                        return <ul>{answer.map((item, index) => <li key={index}>{String(item)}</li>)}</ul>;
                    }
                    if (typeof answer === 'object' && answer !== null) {
                        return <pre className="text-xs font-mono whitespace-pre-wrap bg-slate-200 p-2 rounded">{JSON.stringify(answer, null, 2)}</pre>;
                    }
...
const FormView: React.FC<FormViewProps> = ({ form, allSections, allResponses, onUpdateResponse, currentUser, allUsers, onBack, addNotification, isPreview }) => {
  ...
  const handleSectionSubmit = (sectionId: number, answers: { [questionId: string]: any }) => {
    if (window.confirm("Are you sure you want to submit? This action is final and will lock this section from further edits.")) {
        const responseToUpdate = allResponses.find(r => r.sectionId === sectionId);
        if (responseToUpdate) {
